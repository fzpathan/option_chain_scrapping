...
logging.basicConfig(level=logging.INFO,
                    ...
                    datefmt='%m-%d %H:%M',
                    filename=f'log_Nifty_{datetime.datetime.now().date()}')
...
def get_nf_option_chain():
    ...
    try:
        ...
        opt_chain = opt_chain.rename(columns={'openInterest_CE':'CE_OI','openInterest_PE':'PE_OI',
                                  'changeinOpenInterest_PE':'CHPE_OI','changeinOpenInterest_CE':'CHCE_OI'})
...
def transform_option_chain(engine,a,b):
    ...
    strike = get_strike()
    bnf_strikes = list(range(strike-4*50,strike+4*50,50))
    try:
        df =get_nf_option_chain()
...
def main():
    engine = crate_db_engine(f'NIFTY_{datetime.datetime.today().date()}')
    a,b =0,0
    count = 1
